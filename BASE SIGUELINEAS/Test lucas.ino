...
void loop() {
  ...
  if (sensorValues[2] > umbral && sensorValues[3] > umbral && sensorValues[4] < umbral && sensorValues[5] < umbral) {
    ...
    Motor(-85, 10);
  } else if (sensorValues[3] > umbral && sensorValues[4] > umbral && sensorValues[5] > umbral && sensorValues[6] > umbral && sensorValues[7] > umbral) {
    Motor(50, 50);
    delay(250);
    Motor(-200, 200);
    delay(470);
    Motor(50, 50);
    delay(100);
  } else if (sensorValues[0] > umbral && sensorValues[1] > umbral && sensorValues[2] > umbral && sensorValues[3] > umbral && sensorValues[4] > umbral) {
    Motor(50, 50);
    delay(250);
    Motor(200, -200);
    delay(470);
